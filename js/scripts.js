...
(function ($, root, undefined) {
    ...
    $(function () {
            ...
            $nicer_options.on('click', function(f) {
                ...
                for (var c = 0; c < $checkboxes.length; c++) {
                    ...
                    if ($checkbox.val() == $value ) {
                        ...
                        // call .change so underscore knows its changed
                        $checkbox.prop('checked' ,  !$is_checked  ).change();
        ...
        if (typeof search_url != 'undefined' ) {
            ...
            var $workshops_container = $('#workshops_container');
            ...
            $.ajax({
                ...
                var compiled =  _.template($workshops_template);
...
function processWorkshops(workshops, search, ages, levels, days, disciplines,  locations){
    ...
    if (locations && locations.length > 0) {
        var workshops = _.reject(  workshops ,  function(w) {
            return  !_.contains(    locations  ,  w.centre);
        });
    }
    ...
    var workshops =  _.toArray(workshops)  ;//  CONVERT  OBJECT TO ARRAY


    var previous_workshop_id = false;
    var workshops_visible = [];
    var big_brother_workshop = false;
    ...
    for (var i = 0; i < workshops.length ; i++) {
        ...
        // only show workshops if they are different from the previous
        if (previous_workshop_id != workshop['workshop_id'])  {

            // push to visible workshops, onces processed, also with any sibling workshops
            // once we know there are no more siblings
            if (big_brother_workshop) {
                big_brother_workshop['days'] = big_brother_workshop['days'].join(' ');
                workshops_visible.push(big_brother_workshop);
            ...
            //workshop['api_url'] = search_url   + '?workshop_id=' + workshop.ID;

            var big_brother_workshop = workshop;


        } else {
            if(big_brother_workshop) {
                big_brother_workshop['days'][workshop.day] = '<span>'+ big_brother_workshop['days'][workshop.day] +'</span>';
            }
        }

        previous_workshop_id = workshop['workshop_id'];

    }

    // push the last big brother, because it wont have been added
    if (big_brother_workshop) {
        big_brother_workshop['days'] = big_brother_workshop['days'].join(' ');
        workshops_visible.push(big_brother_workshop);
        big_brother_workshop = false;
